...
class MeanAggregator(Layer):
    ...
    def __init__(self, input_dim, output_dim, neigh_input_dim=None,
            ...
            name=None, concat=False, num_heads=1, sample_num=1, **kwargs):
    ...
    def _call(self, inputs):
        self_vecs, neigh_vecs = inputs
        self_vecs = tf.contrib.layers.layer_norm(self_vecs)
        neigh_vecs = tf.contrib.layers.layer_norm(neigh_vecs)
        #self_vecs = tf.layers.batch_normalization(self_vecs)
        #neigh_vecs = tf.layers.batch_normalization(neigh_vecs)
